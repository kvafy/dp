...
So far we have seen how to compute CPDs for a given graph structure of a BN based on some dataset. Learning the structure itself is probably the most complicated task. One of the problems is to pick a good trade-off between high bias (restricting complexity of the graph structure) and high variance (allowing more edges to get a better fit for training data). As we have already discussed, with small dataset we may actually benefit by having a simpler structure. On the other hand, with large dataset we don't want to restrict our hypothesis too much because the dataset is sufficient to correctly learn larger number of parameters.

In this chapter we are going to discuss \term{score-based} methods. These methods use a single metric to evaluate a network structure. Because the problem of finding ideal structure of a~BN is NP-complete, heuristics are usually used instead. Structure learning is basically an optimization task and, in general, works as follows. We have an initial graph structure with fixed set of variables and no edges (no dependencies). Then we try all possibilities of adding an edge an evaluate the change using a predefined metric. We accept the change with the best gain and repeat these actions until the solution improves. The metric then measures both degree of dependency of the variables connected which is desirable, as well as increase of the network complexity which we want to keep low.

\subsection{Likelihood score and BIC score}

%We need to stress out that a fully connected network is the most expressive and can capture joint probability distribution induced by any BN. Let's consider that the dataset has been sampled from a distribution $P*$ represented by a BN with structure $\mathcal{G}$. Then any supergraph of $\mathcal{G}$ 

\begin{itemize}
	\item Why? Discover the structure (dependencies) or to really use the network to answer queries
	\item fewer edges can generalize better than too many edges
	\item score-based learning: define score for structure (how well it matches the data)
	\item $\hat{P}(\cdot)$ is \term{empirical distribution}, ie. simple relative counts (exactly as in maximum likelihood estimation)
	\item lecture Likelihood scores is pretty good to understand (maybe include the derivation?)
	\item likelihood score almost always positive (indicates to add an edge)\,--\,avoid overfitting (restrict no of parents or parameters), score that penalizes complexity
	\item BIC score is likelihood score with a penalty term (for complex structures)
	\item 
\end{itemize}
...
tree structures, general graphs
