...
For structure learning based on local state-space search using the BIC score I use twofold caching\,--\,for delta scores of possible alterations and for mutual information estimates. Caching is important because in order to evaluate the BIC score we have to make a pass through the entire dataset which may be very large. Furthermore, for two networks differing only by a single alteration, most of the possible alterations will yield the same delta score or will be at least partly computable using values that have been already determined in the previous step of local search.

The caches need to be maintained at minimum size to remain effective and the trick is to keep only those values, that will be needed and dropping those that won't. To achieve the correct dropping behavior of the cache keeping delta scores for possible alterations we need to understand how the delta score is computed for the three possible alterations\,--\,edge addition, removal and reversal (for reference see the BIC score definition~\eqref{eq:bic_score}):
\begin{itemize}
	\item For addition or removal of edge $(X,Y)$ the delta score is the increase of mutual information $I_{\hat P}(Y,Parents(Y))$ minus the increase of the dimension term $\log(N) Dim[G] / 2$. So, whenever the action taken in the current step of local search changes the set $Parents(Y)$, all cached values for actions add or remove $(X,Y)$ have to be dropped.\medskip \\
...
\medskip
The mutual information cache is useful because if we take some action that modifies say $Parents(X)$, the cached increase of mutual information change for some actions is invalidated as explained earlier.
\todo{so what?}

The cache of mutual information queries is implemented as a caching proxy \srccode{CachedDataset} of the \srccode{Dataset}.
...
\end{equation*}
As we can see, KL-divergence nicely decomposes over a BN so that we make use of plain CPD entries rather than inspecting probability of each atomic event, however probability for parents assignment $P(\vars{pa})$ needs to be inferred. %Luckily, I am entitled to use stochastic inference only.
From mathematical point of view, the KL-divergence is defined only if for any $x_i$ and any $\vars{pa}$ always holds that $\bigl( Q(x_i \mid \vars{pa}) = 0 \bigr) \Rightarrow \bigl( P(x_i \mid \vars{pa}) = 0 \bigr)$, ie. we can't have non-zero $P$ probability and zero $Q$ probability at the same time. However, in practice this is impossible to guarantee when computing network parameters using maximum likelihood estimation with a small dataset. According to~\cite{x,y}, the normal practice is to artificially set a small positive lower bound for $Q$ probabilities which is therefore what I do.
