...
This chapter will present theoretical foundations for understanding Bayes networks and for doing computations over them. Studying Bayes networks (further abbreviated as BNs) is challenging both because of necessary mathematical rigor as well as for the need of efficient non-trivial algorithms to reason about Bayes network.

First will be presented notation used in this thesis and necessary overview of basic statistics. Then we will proceed to theory of Bayes networks, at first in terms of pure mathematics followed by practical and effective \uncertain{reasoning about information represented by them.}




\section{Preliminaries}
At first we need to establish notation and explain basic facts regarding mathematical probability. As will be shown later, Bayes networks are models for discrete probability distributions.
...
Why use Bayes networks? As was illustrated earlier having a complicated joint probability distribution with many random variables inevitably leads to a potentially huge table that would represent such joint probability distribution. Lets assume we have random variables $X_1, X_2, \dots$ and each of these variables is binary. Then for representing distribution $P(X_1)$ we would need a table with two entries, for distribution $P(X_1, X_2)$ the table would have four entries and a general distribution $P(X_1, \dots, X_k)$ would have a table with $2^k$ entries which brings several problems:
\begin{itemize}
    \item Table describing the joint probability distribution may be too big to store.
    \item Even if the table could be stored, making calculations over it would not be efficient.
    \item In order to construct such a probability table:
    \begin{enumerate}
        \item[a)] We would require huge amount of data to create an accurate statistical model from since we would essentially count occurrences of every single possible assignment to our random variables and finally divide it by the total number of all examples.
...
As will be shown later, Bayes networks couple with these problems by having several smaller probability distributions (factors) based on dependencies between random variables.
