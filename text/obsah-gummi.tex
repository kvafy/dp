...
\subsubsection{Likelihood weighting (\todo{implementation: compute $P(\vars{X}\cup\vars{Y}\mid\vars{e})$ and renormalize})}
Rejection sampling described earlier can theoretically be used for answering queries of the form $P(\vars{X} \mid \vars{e})$ but the number of rejected samples may be unbearable in practice. Likelihood weighting method copes with this problem by forcing every sample to be consistent with observed evidence. Then, of course, not every sample is equally likely, so we cannot determine $P(\vars{x} \mid \vars{e})$ by simply dividing number of samples $N_{\vars{x},\vars{e}}$ by $N_\vars{e}$. For each sample we need some additional information which is the probability of assigning values $\vars{e}$ to evidence variables given other values in the current sample~\cite[p.~514]{russell_norvig_ed2} (ie. given values of parents). Formally written, weight of a sample $\vars{x},\vars{e}$ is $w_{\vars{x},\vars{e}} = \prod_\vars{E} P(e \mid parents(E))$.

The sampling algorithm used in direct sampling needs to be changed in the following way. When generating a~new sample, we set weight of the current sample to 1. Then, for a non-evidence variable we proceed as before\,--\,randomly assign a value according to the probability distribution of that variable and according to the values currently assigned to its parents. When we encounter an evidence variable $E_i$, we set it to value $e_i$ according to the evidence $\vars{e}$ and multiply weight of the current sample by $P(e_i \mid parents(E_i))$. The final probability distribution is computed as follows:
