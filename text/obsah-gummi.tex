...
To explain how the MCMC algorithm works we first need to introduce so called \term{Markov blanket} of a~variable $X$ (see Figure~\ref{fig:bn-markov-blanket}), denoted $MB(X)$. The Markov blanket of $X$ is a minimal set of variables such that $X$ is conditionally independent of all other variables in the BN given $MB(X)$. It is fairly obvious that the Markov blanket includes variables $Parents(X)$ and direct children of $X$, denoted $Children(X)$, because there is a direct connection between these variables and $X$. Furthermore, $MB(X)$ also needs to include parents of every variable in $Children(X)$ because observing any of the children $C_i$ activates a V-structure and $X$ becomes conditionally dependent on $Parents(C_i)$ given $C_i$. Formally the Markov blanket is defined as follows:
\begin{equation*}
...
The MCMC algorithm~\cite[p.~516]{russell_norvig_ed2} starts by producing one sample consistent with the evidence, eg. by one iteration of likelihood weighting with the weight discarded. Then we repeat the following: For each non-evidence variable $Y$ we resample this variable from the current distribution $P(Y \mid mb(Y))$ producing a new sample with value of variable $Y$ potentially changed. For the new sample we increase the corresponding counter $N_{\vars{x},\vars{e}}$ and proceed with resampling of another variable. The distribution $P(Y \mid mb(Y))$ needed for resampling of variable $Y$ can be obtained by computing $P(y \mid mb(Y))$ for every $y$ as follows:
\begin{equation*}
    P(y \mid mb(Y)) = \alpha \cdot P(y \mid parents(Y)) \!\!\!\!\!\!\!\! \prod_{C_i \in Children(X)} \!\!\!\!\!\!\!\!\!\!\!\! P(c_i \mid parents(C_i))
\end{equation*}
...
\end{itemize}
\todo{Extent of this chapter is expected to be around 10 pages.}



\section{Data structures}
\todo{\srccode{Factor, Node, BayesianNetwork} etc.}
...
\subsection{Multithreaded sampling}
\todo{\srccode{java.util.Random} cannot be shared among threads because it (1)~degrades computation with $n$ threads to virtually running the sampling $n$-times in a single thread and (2)~\srccode{Random} doesn't appear to be thread safe as it leads to invalid results when shared among threads (synchronized implementation of \srccode{Radnom} was proved to solve this issue)}
