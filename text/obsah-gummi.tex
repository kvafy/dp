...
\section{Sampling}
Sampling is implemented by classes in the \srccode{bna.bnlib.sampling} package. Basically, there are two parallel hierarchies of objects which will be further described in detail (see also class diagram in \todo{Figure~\ref{todo}}).
...
\todo{Weighted sampling can prune the leaf nodes not in $X \cup Y \cup E$ and none of their descendants in this set $\Rightarrow$ faster computation}
