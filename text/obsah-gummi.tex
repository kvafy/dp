...
\note{pgm 541+ has good notes of other papers}
...
\todo{maybe include general metrics?}
...
Again, the goal is to maximize probability of the data given a hypothesis. Let's suppose we have a dataset $\mathcal{D} = \lbrace \xi_1, \dots, \xi_m \rbrace$ and let's examine network $\mathcal{G}_1$ with two independent variables $X$ and $Y$ and network $\mathcal{G}_2$ with two independent variables and structure $X \rightarrow Y$. Then their likelihood scores are computed as follows:
\begin{eqnarray*}
...
The term $I_{\hat P}(X, Y)$ is called \term{mutual information} between $X$ and $Y$ and expresses the average distance between the joint distribution $P(X,Y)$ when the variables are dependent to the distribution given as product of marginal distributions $P(X)$ and $P(Y)$ when the variables are independent. It can be proofed (for details see~\cite[p.~792]{pgm}) that the overall likelihood score decomposes over a general graph $\mathcal{G}$ as follows:
\begin{equation}
...
\end{equation}
where $Dim[\mathcal{G}]$ is dimension of the network (number of its parameters) and the first two sums is the likelihood score. We can see that the BIC score increases linearly in $N$ with dependent variables being connected (the first term) and decreases logarithmically in $N$ with the network complexity (the third term). Thus for a sparse dataset only the strong dependencies will be present in the final network, whereas for a large dataset the penalization increases rather slowly allowing the network to have a more complicated structure with weaker dependencies.
...
\begin{itemize}
	\item What data structures are used for representing factors, CPDs, nodes and the overall Bayesian network.
	\item Algorithms needed for effective inference and learning including their modifications and tweaks.
...
\end{itemize}
Extent of this chapter is expected to be 5-10 pages.
...
This chapter will present collection of selected applications of Bayesian network learning. We will always describe the problem itself, reason why it is interesting, approach to its solution and the achieved results. Currently, candidate applications are the following:
