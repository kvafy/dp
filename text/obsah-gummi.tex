...
This chapter will present theoretical foundations for understanding Bayesian networks and performing computation over them. Studying Bayesian networks (further abbreviated as BNs) is challenging both because of necessary mathematical rigor as well as for the need of efficient non-trivial algorithms to construct them and to provide answers for probability queries.

First will be established notation used in this thesis and necessary overview of probability theory. Then we will move on to the theory of Bayesian networks, explain their advantages and proceed to construction and inference techniques.
...
An important drawback of the maximum likelihood estimation presented earlier is that it doesn't really account for the size of the dataset. The key idea of Bayesian estimation is to define a prior distribution over parameters $\Theta$ with some degree of strength of this initial distribution and then compute the posterior probability distribution $P(\Theta \mid \mathcal{D})$ as we get more empirical data. This way we can, for example, distinguish between rolling a six three times out of eight throws and rolling a six 300 times out of 800 throws. It is clear that these two datasets tells us something different about the die, although probability of rolling a six is, according to maximum likelihood estimation, $3/8$ in both cases.

The exact mathematical derivation~\cite[p.~733]{pgm} of Bayesian parameter estimation formulas is based on \term{Beta} distribution. The derivation is not trivial and also is not needed further in this thesis, so we jump straight to the practical conclusion. For a given dataset $\mathcal{D}$ and hyper-parameters $(\alpha_1, \dots, \alpha_k)$ the probability of variable $X$ having value $x_i$ is given as follows:

\begin{equation*}
    P(X = x_i \mid \mathcal{D})
     = \frac
        {\alpha_i + N_{x_i}}
        {\sum_j \alpha_j + \sum_j N_{x_j}}
\end{equation*}

\todo{ratios of alphas govern the initial distribution, sum over alphas is "strenght" of the prior distribution (how may samples does it take to change the posterior), for alphas $(1,\dots,1)$ it is Laplace's correction and uniform distribution, asymptotically the same as MLE}
...
\begin{itemize}
	...
	\item learning as a special type of inference (prior is what we expect the distribution to be $P(\Theta)$ and posterior is $P(\Theta \mid \mathcal{D}) = P(\Theta) \prod_{X_i \in \mathcal{D}} P(X_i \mid \Theta)$)
