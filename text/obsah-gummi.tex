...
Important drawback of likelihood weighting is that it doesn't account well for evidence in leaf nodes or near to them. This is because in that case we effectively sample the prior probability distribution and finally we modify weight of the generated sample by evidence but the sampling process is most of the time unaffected by the evidence and virtually only weights of samples take evidence into account. So, for a very rare instantiation of evidence variables (eg. for some rare and/or big set of symptoms) all of the generated samples might have very small weight and, in case of medical diagnosis, no sample representing the \uv{real cause} of observed evidence might be generated at all. Once again, this is because we sample prior probability distribution rather than posterior and these two distributions may be very different~\cite[p.~503]{pgm}.
...
Basically, there are two approaches to creating a BN. First option is to cooperate with an expert of target domain who can make correct dependency and independency assumptions and provide us with conditional probability distributions, ie. with CPDs to all nodes. The other approach is to use automated techniques for creating models based on a big dataset. The dataset can be viewed as a set of samples of the target probability distribution $P*$ which we attempt to reconstruct. The aim is to create a model $\tilde{\mathcal{M}}$ such that the probability distribution $P_{\tilde{\mathcal{M}}}$ induced by $\tilde{\mathcal{M}}$ is \uv{very close} to the original distribution $P*$.
...
\begin{eqnarray*}
   score_L(\mathcal{G}_2 : \mathcal{D}) - score_L(\mathcal{G}_1 : \mathcal{D}) 
     ...
     & = & N \cdot I_{\hat P}(X; Y)
\end{eqnarray*}
First, I will explain the derivation step by step and the next paragraph will address the question of usefulness of the derived result. The first step (performed on the second line) is to change the logic of the summation from \uv{over the dataset $\xi_1,\dots,\xi_m$} to \uv{over possible instantiations of $X,Y$}; such change is legal because each $\hat P(\cdot)$ value will be taken into account just as many times as before.
...
The term $P(\mathcal{D} \mid \mathcal{G})$ in equation~\eqref{eq:bayesian_score_general} will be our major concern. It is a marginal distribution (because all possible parameter settings $\Theta$ are marginalized out) usually defined in the following integral form:
...
\begin{equation}\label{eq:bayesian_score_for_bn}
    P(\mathcal{D} \mid \mathcal{G})
      = \prod_{X_i \in Nodes}
             ...
             \Biggr]
\end{equation}
where $Val(\vars{X})$ is the set of all instantiations $\vars{x}$ of variables $\vars{X}$, $\alpha_{X \mid \vars{pa}} = \sum_{x \in Val(X)} \alpha_{x \mid \vars{pa}}$, $N_{x,\vars{pa}}$ is number of samples for which $X = x \land Parents(X) = \vars{pa}$ and similarly for $N_\vars{pa}$.
...
In practice, we take a logarithm of~\eqref{eq:bayesian_score_for_bn} because computation of $log \, \Gamma(x)$ can be numerically computed more efficiently than $\Gamma(x)$~\cite[p.~801]{pgm}.
...
\begin{comment}
Let $n$ be the number of variables (for asymptotic running-time analysis).
\begin{itemize}
	\item Remove edge $(X,Y)$: Possible if $(X,Y) \in Net$.
	\item Add edge $(X,Y)$: Beware of creating a cycle.
	    \begin{itemize}
	        \item[a)] Generate adjacency matrix in $O(|V| + |E|) = O(n + n^2)$ and compute $r^+$ using Warshall algorithm in $O(n^3)$. Edge $(X,Y)$ can be added if $(Y,X) \not\in r^+$, this is done in $O(n^2)$ over all tuples $(X,Y)$. Total time of $O((n + n^2) + n^3 + n^2)$.
	        \item[b)] Start a parent-to-children DFS from $Y$ and if $X$ is reachable, adding $(X,Y)$ would introduce a cycle. $O(n^2) \cdot O(|V|+|E|) = O(n^2) \cdot O(n + n^2) = O(n^3 + n^4)$.
        \end{itemize}
	\item Reverse edge $(X,Y)$: Beware of creating a cycle and, if possible, make use of $r^+$.
	    \begin{itemize}
	        \item[a)] Start a child-to-parents DFS from $X$ and\dots
	        \item[b)] Let $\prec$ be ordering induced by a topological sort of the BN computed in time $O(|V|+|E|) = O(n + n^2)$. Then for all nodes we can write $A_1 \prec \dots \prec A_a \prec X \prec M_1 \prec \dots \prec M_m \prec Y \prec B_1 \prec \dots \prec B_b$ which can be written using sets of variables as $\vars{A} \prec X \prec \vars{M} \prec Y \prec \vars{B}$ where any of the sets may be empty. Edge can be reversed iff $\not\exists M \in \vars{M}: Xr^+Mr^+Y$ (ie. $M$ that is strongly connected with $X$ and $Y$ at the same time). Equivalently $(X,Y)$ may be reversed iff $\vars{M}$ can be made empty by moving all of its variables directly before $X$ or after $Y$ in the $\prec$ ordering while it still being a topological sort.
        \end{itemize}
\end{itemize}
\end{comment}
...
The GUI application (not the \srccode{bnlib} library itself) relies also on the following third party packages:
\begin{itemize}
	\item \srccode{ini4j-0.52} package for reading and writing configuration \srccode{.ini} files (released under Apache License v2.0).
...
Also, according to the tests performed, MLE might seem to be steadily outperformed. Of course, if we had an excessive amount of training data, which is usually not the case in practice, then MLE would actually be more suitable than Bayesian estimation because the pseudosamples representing prior distribution would be a hard-to-overcome source of inaccuracy. Nonetheless, if a concrete application demands estimation of network parameters which are as close to the \uv{real ones} as possible, similar analysis as I just performed is a~sensible thing to do\,--\,pick a network of similar complexity, plot the learning curves for this network and determine what is the best parameter estimation method for dataset of given size.
...
Evaluation of structure learning proved to be a little problematic. We cannot use KL-divergence because the network structures aren't identical and other metrics for measuring distance of two probability distributions aren't suitable as they are based on exhaustive enumeration of atomic events. Instead, I rely on manual inspection of the best-scoring network structures found over multiple runs of the learning algorithm. This approach is perfectly legal as I want to demonstrate usage of Bayesian networks in the field of data-mining which is largely human-assisted. As I will explain shortly, there are usually more structures sharing the same highest score. As we don't know the \uv{true} structure, we need to inspect all best-scoring structures. To ease the evaluation by hand I also maintain statistics for each pair of variables $U, V$ how often the final learnt structure contains the edge $U \rightarrow V$. This statistics is presented by a table in GUI.

One potential problem I have already mentioned is that typically more than one network structure can capture the exact probability distribution induced by the \uv{true} network. Strictly speaking, a network structure can capture desired distribution $P$ if this network structure doesn't imply any independency that isn't implied by the \uv{true} network that induces $P$. This notion is called \term{I-equivalent} structures (as in independency equivalent)~\cite[p.~76]{pgm}. As an example, consider three simple networks $A \rightarrow B \rightarrow C$, $A \leftarrow B \rightarrow C$ and $A \leftarrow B \leftarrow C$. In these three networks holds one and only one conditional independency $A \perp C \mid B$ and no unconditional independency, therefore these three networks are I-equivalent. As a result, these networks are capable of representing the same set of probability distributions and also have the same BIC score~\cite{pgm}. Practically speaking, these three network structures are equally good as a result of structure learning because there is really no causal relationship between them. Further in this section I will refer to such configurations as \term{chains} $X_1 - \dots - X_n$ which will generally be variables $X_1, \dots, X_n$ connected in a linear fashion in specified order with no V-structure and with no incoming edges to any of the variables. I also introduce graphical notation for chains in Figure~\ref{fig:structure_learning_chain}. Another set of I-equivalent structures that sometimes arises I call \term{central variable} meaning there is a \uv{central} variable $X$ and a set of variables $Y_1,\dots,Y_n$ directly connected to $X$. At most one $Y$ variable is direct parent of $X$ while all the other $Y$ variables are direct children of $X$. All such structures are also I-equivalent. Please see graphical notation for central variable in Figure~\ref{fig:structure_learning_central_variable}.

\begin{figure}[ht]
    \includegraphics[scale=0.4]{fig/structure-learning-chain}
    \caption{A general chain $X_1 - X_2 - \dots - X_n$ of variables $X_1, \dots, X_n$ is a set of network structures where the variables are connected in a linear fashion in specified order and there is no V-structure. Left side of this figure demonstrates the full set of $n$ structures for the same chain and on the right side of this figure there is an efficient graphical notation for the whole set of chains (bounded structure without directed edges).}
    \label{fig:structure_learning_chain}
\end{figure}

\begin{figure}[ht]
    \includegraphics[scale=0.4]{fig/structure-learning-central-variable}
    \caption{Central variable structure $X,Y_1,\dots,Y_n$ is a set of I-equivalent structures where the $X$ variable is central while at most one $Y_i$ variable is parent of $X$ and all the other $Y_i$ variables are children of $X$. There also may not be an incoming edge to any of the variables $X$ or $Y_i$ as that would potentially introduce a V-structure in some of the networks.
Unbounded structures are the full set of $n+1$ central variable structures and on the right side of this figure there is an efficient graphical notation for the whole set of central variable structures (bounded structure without directed edges).}
    \label{fig:structure_learning_central_variable}
\end{figure}

Before we proceed to the experiments with learning structure of various network please note that the outcome of the experiments relies on the concrete dataset we sample before learning. So, for two datasets of equal size we quite frequently obtain different sets of best-scoring structures, especially when the number of samples is small. \todo{Save the \uv{right} datasets to a directory to ship on CD with the final thesis.}
...
With a dataset of 1000 samples the structure learning algorithm discovers only dependency among four variables which form a substructure where three variables are directly connected to the fourth variable and avoid to form a V-structure as that would introduce a conditional dependency (see Figure~\ref{fig:structure-learning-cancer-1000}). The fifth variable, $Pollution$, is isolated in the found structures as $P(Cancer \mid Pollution)$ differs only by the value $0.03$ for different assignments of $Pollution$ and the potential edge $Pollution \rightarrow Cancer$ is in BIC score more penalized by the increase of structure complexity than it is favored for its contribution to mutual information sum.

For a dataset with 5000 samples we finally obtain a connected graph, only there is similar situation as we had with 1000 samples\,--\,$Cancer$ variable is in the middle and connected with all the other variables while avoiding to form a V-structure (see Figure~\ref{fig:structure-learning-cancer-5000}). It seems that V-structures need to be strongly implied by the dataset, otherwise the learnt structure won't contain them.

Finally, with a dataset of 15000 samples the learning process yields the exact original structure of cancer network and no others (see Figure~\ref{fig:structure-learning-cancer-15000}). The original structure is unique as it contains a V-structure in $Cancer$ variable which spoils any possibility for there being a chain of variables that could be somehow rearranged.
...
\begin{figure}[ht]
    ...
    \begin{subfigure}[b]{0.45\linewidth}
        ...
        \caption{Dataset of 1000 samples, 4 I-equivalent structures.}
    ...
    \begin{subfigure}[b]{0.45\linewidth}
        ...
        \caption{Dataset of 5000 samples, 5 I-equivalent structures.}
        \label{fig:structure-learning-cancer-5000}
    \end{subfigure}
    \quad
    \begin{subfigure}[b]{0.45\linewidth}
        \centering
        \includegraphics[scale=0.4]{fig/structure-learning-cancer-15000}
        \caption{Dataset of 15000 samples, single structure (exactly the original cancer network).}
        \label{fig:structure-learning-cancer-15000}
    \end{subfigure}
    \caption{Best-scoring I-equivalent network structures found for the cancer network with datasets of various sizes.}
    \label{fig:structure-learning-cancer}
\end{figure}


\subsubsection{Experiments with Asia network}
The Asia network contains 8 variables and a single V-structure $either \rightarrow dysp \leftarrow bronc$. The CPD for $dysp$ variable suggest strong dependency on both its parents\,--\,the distribution $P(dysp \mid either, bronc)$ changes up to $0.7$ for different assignments of $either$ and up to $0.6$ for different assignments of $bronc$. Therefore for as little as 100 samples the found networks contain the V-structure at $dysp$ variable. Furthermore, $dysp$ has always the same set of parent variables but that can be accounted to the strong dependency in the original network.
...
\begin{figure}[ht]
    ...
    \end{subfigure}
    \caption{Best-scoring I-equivalent network structures found for the asia network with datasets of various sizes.}
...
With a dataset of 500 samples there are two isolated variables $Age$ and $BirthAsphyxia$, the rest of the network is almost surprisingly strongly consistent with the original network, considering how little samples we have. The best-scoring networks contain four V-structures out of the six which are present in the original network, again suggesting that once a V-structure is learnt, it is a fairly stable feature of the network (unlike a chain or a central variable structure). When not considering edge directions the learnt network is missing only 2 out of 25 connections present in the original network and from the remaining 23 edges only one is reversed. Furthermore, in the learnt network there isn't any redundant connection that wouldn't be in the original network. Although I~believe this to be a great success, we must take into account that this network has such a structure that there aren't any chain nor central variable structures except for chain $BirthAsphyxia - Disease$ and therefore the number of reversed edges is so small.

With a dataset of 2000 samples we get a connected graph in which only one edge $Sick - Age$ is missing and two or three other edges are reversed. Although the set of reversed edges is different for the found best-scoring structures, their number remains effectively the same, suggesting that if we were to use structure learning for causal discovery, the resulting best-scoring structures are equal in terms of how accurately they resemble the \uv{true} network, assuming they are not I-equivalent to the \uv{true} network.

Dataset of 10000 samples gets us as close to the original network as only two reversed edges without any other structural mistakes.



\subsubsection{Experiments with ICU Alarm network}
ICU alarm network is with its 37 variables the largest network I experiment with in this section. There are many two-node chains (eg. $PMB - PAP$, $APL - TPR$ or $MVS - VMCH$) meaning there are quite a few structures I-equivalent with the original network. On the other hand, there are many V-structures and the assumption is that V-structures are relative stable.
