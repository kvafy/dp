...
My notes:
\begin{itemize}
    \item Data representation: vector space model (each dimension represents a word), we need to discretize the data (equi-depth binning  didn't work, used k-means instead with $k=3$)
    \item split 80:20 train:test (3681:920), stratified sampling (with RapidMiner)
    ...
    \item \todo{Find the biggest dataset possible.}
    \item Use an already pre-processed dataset as the preprocessing textual documents for datamining isn't concern of this thesis.
    \item Discretization: wanted equi-depth binning but turned out some attributes are very unevenly distributed, so two bins were often the same. Used k-means clustering ($k=3$) instead.
    ...
    \item Dataset \uv{Spambase} from UCI Machine Learning Repository \url{http://archive.ics.uci.edu/ml/datasets/Spambase}.
\end{itemize}
