...
Why use Bayesian networks? First of all, having a complicated joint probability distribution with many random variables inevitably leads to a potentially huge table that would represent such probability distribution. Lets assume we have binary random variables $X_1, X_2, \dots$ Then for representing distribution $P(X_1)$ we would need a table with two entries, for distribution $P(X_1, X_2)$ the table would have four entries and a general distribution $P(X_1, \dots, X_k)$ would have a table with $2^k$ entries. This exponential growth brings several problems:
\begin{itemize}
    \item Table describing a joint probability distribution may be too big to store.
    \item Even if the table could be stored, making calculations over it (e.g. factor marginalization or multiplication) would not be efficient. In fact operations such as exact inference are known to be NP-hard~\cite{pgm} in number of variables.
...
\begin{math_def}
    Let $G$ be a Bayesian network, $X,Y$ two of it's nodes and $Z$ facts (observations). We say that $X,Y$ given $Z$ are d-separated in $G$ (denoted $\text{d-sep}_G(X,Y|Z)$) iff there is no active trail between $X$ and $Y$ given $Z$.
\end{math_def}

\todo{$P(X|Parents(X)$ means, by the way, that $X$ is independent of its previous ancestors given its parents}

Using d-separation (direction-dependent separation~\cite{russell_norvig_2ed}) we can quite elegantly determine if an observation of some variable $X$ can or cannot affect distribution of other random variable. Basically variable $X$ can influence variable $Y$ given facts $Z$ if there is an active trail between $X,Y$ given $Z$ (or equivalently if $X,Y$ aren't d-separated given $Z$). So it boils down to the question whether there is flow of influence between some variables.
...
There are several types of queries that might be asked in context of a Bayesian network. Most often by inference in Bayesian networks is meant computing the posterior probability distribution for a set of query variables given values of evidence variables. Mathematically the task is to compute the probability distribution $P(Query | Evidence)$. Another type of query is so called \term{maximum a~posteriori} (abbreviated as MAP) whose task is to find $arg\ max_X P(X|E)$. MAP is basically used for finding out the most probable cause (concrete assignment of variables $X$) for observations $E$. Practical example could be getting the most probable word (sequence of letters) that corresponds to classifier output for a segmented picture containing that word.

Inference in a BN may be carried out in many ways depending on nature of the query and precision requirements. Exact inference is relatively straightforward in terms of mathematical description and algorithmic implementation but exhibits serious running-time disadvantages. On the other hand, approximate inference methods allow us to answer queries more quickly what we get isn't an exact solution.
...
Probably the simplest method for computing a conditional probability query is called sum-product. The problem at hand is to compute conditional probability distribution $P(X | e)$ where $X$ is one or more variables, $E$ are evidence (observed) variables and $Y$ variables that are neither query nor evidence variables. The query is computed according to Bayes' theorem as follows:
\begin{equation*}
P(X|e)
 = \frac{P(X, e)} {P(e)}
 = \frac{\sum_Y P(X, y, e)}{\sum_{X,Y} P(x, y, e)}
\end{equation*}
The denominator is constant and since $P(X|e)$ has to be a probability distribution, we can simply omit the denominator and in the end normalize numerator by some constant $\alpha$. Hence we can write
\begin{equation*}
P(X|e)
 = \alpha \sum_Y P(X, y, e)
\end{equation*}
Computation of this expression using the sum-product method means making four steps:
\begin{enumerate}
	\item reducing all factors in the Bayesian network by evidence $e$,
	\item multiplying reduced factors all together producing single factor that represents "unnormalized joint probability distribution $P(X,Y,e)$" (which in fact is not a probability distribution),
	\item marginalizing over non-query and non-evidence variables $Y$ and finally
	\item renormalizing the resulting factor, obtaining the joint probability distribution $P(X,e)$.
\end{enumerate}


Every item of the sum in numerator can be written according to joint distribution represented by a BN (definition~\eqref{eq_bn_joint}) as a product of conditional probability distributions of all nodes in given BN (equivalently of all factors). This explains the name of this inference method\,--\,we effectively compute many products of factors and sum them.

Sum-product inference algorithm has, from its nature, exponential running time in number of variables because it effectively generates the whole joint probability distribution encoded by given Bayesian network and size of a table representing any joint probability distribution has been earlier shown to be exponential wrt. number of variables. Because of this quality is the sum-product method suitable only for inference in considerably small networks.

\uncertain{ (je třeba rozlišit operaci násobení faktorů a násobení uvnitř faktorů samotných)

Formally, exact number of operations carried out in the nominator sum can be determined as follows. Let $m$ be the number of all variables, $n$~be the number of non-query and non-evidence variables $Y$ and let $k$~be the number of possible assignments to a variable. The numerator summation unrolls to $k^n$ items, each item being a product of all $m$ factors which means $k^n (m-1)$ multiplications and $k^n - 1$ additions. Therefore the overall running time is roughly $O(m k^n)$.}

\subsection{Variable elimination}
Variable elimination algorithm can be seen as a somewhat smarter implementation of the sum-product algorithm explained earlier. The main ideas behind variable elimination are:
\begin{itemize}
	\item Suppose we sum over many variables in a product of factors, say one of these variables is $X$. Then factors whose scopes don't contain $X$ are irrelevant for the summation over $X$ and hence these factors can be factored out in front of the summation over $X$. This way we can push all summations as far to the right as possible, so that each summation is done only over factors containing the variable being summed out. This effectively eliminates reevaluation of some factors and thereby speeds up the computation. \todo{concrete example}
	\item \todo{$\sum_X P(X|E) = 1$ by definition, so any node that is not an ancestor of a query or evidence variable can be left out. Implementation can be recursively done in the way of eliminating all leave nodes that are not in $X \cup E$ and performing the same operation until some node is removed.}~\cite{russell_norvig_ed2, 509,537pdf}
	\item \todo{Running time is governed by the greatest immediate factor that arrises during the computation, so we can choose a better ordering of factor products and marginalizations. The problem is NP-complete, so heuristics are used instead~\cite{pgm}.}
\end{itemize}

\todo{řeči kolem, algoritmus, časová složitost, kliky, časová složitost dána největším immidiate faktorem}

\todo{Running time in polytrees is $O(n)$~\cite{russell_norvig_ed2,510,538pdf}, therefore it's good method for answering individual queries $P(X|e)$ where $X$ is single variable (or small number). Running time in multiply connected networks has exponential running time in general but it can be helped with by clustering algorithms that transform multiply connected network to a polytree.}
...
\cite{russell_norvig_ed2,514,pdf542}

Likelihood sampling is an approximate inference method from Monte Carlo family. Other less sophisticated Monte Carlo methods include \term{direct sampling} and \term{rejection sampling}. These methods won't be discussed in detail.
