...
\begin{enumerate}
    ...
    \item For probability distributions of variable $X$ and its representative $R$ has to hold $P(ViolentCrimesPerPop \mid X) \approx P(ViolentCrimesPerPop \mid R)$, of course, with some tolerance. These probability distributions will be inferred by sampling after the network parameters are learnt using Bayesian estimation with $\alpha_P = 30$. The value of $\alpha_P$ has been chosen based on the KL-divergence analysis of parameter learning with the Hepar-II network\,--\,this network is the most similar I could find to the learnt networks and also gives us a good reserve while the learnt networks get simpler through gradual elimination of attributes. In Figure~\ref{fig:kl_div_hepar2} we can see that for 2\,000 samples, which is the size of our dataset, the value $\alpha_P = 30$ gets us closest to the original probability distribution.
...
\begin{itemize}
    ...
    \item $PctPertOwnOcc, PctHousOwnOcc$
\end{itemize}

Each of these sets contains variables that are topologically close in the learnt structure (directly or transitively connected within the set), semantically similar and the probability distributions $P(ViolentCrimesPerPop \mid X)$ and $P(ViolentCrimesPerPop \mid Y)$ for any two variables $X,Y$ within the same set differ at most by 0.05 for any instantiation. For the next iteration only the first variable of each set is preserved which eliminates 13 attributes.

\todo{in the second iteration\dots}
