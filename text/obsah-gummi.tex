...
Bayesian networks represent a subclass of probabilistic graphical models and serve as a~tool for modeling probability distributions of random variables. Bayesian networks were originally used to evaluate medical data of a patient in order to determine the probability of them having a certain disease\,--\,a situation with many unknown facts (eg. missing medical history or unknown results of possible tests). Since then there have been many other applications such as inspection of pedigree trees in genetics or general classification tasks.
...
The aim of this thesis is to present techniques of learning Bayesian networks, both in terms of conditional probability tables as well as in terms of structure and to show possible application domains. Inference is narrowed down to stochastic approaches because other ways are very complex. Still all forms of inference will be mentioned at least on the level of basic concept, since it is a fundamental part of theory of Bayesian networks.
...
More formally, we define a~prior distribution $P(\Theta)$ over parameters $\Theta$ with some degree of strength of this initial distribution and then, using the Bayes' rule, compute the posterior probability distribution $P(\Theta \mid \mathcal{D}) \propto P(\mathcal{D},\Theta) P(\Theta)$ as we get more empirical data. This way we can, for example, distinguish between rolling a~six three times out of eight throws and rolling a six 300 times out of 800 throws. It is clear that these two datasets tell us something different about the die, although probability of rolling a six, according to maximum likelihood estimation, is $3/8$ in both cases.
...
Again, the goal is to maximize probability of the data given a hypothesis. Let's suppose we have a dataset $\mathcal{D} = \lbrace \xi_1, \dots, \xi_m \rbrace$ and let's examine network $\mathcal{G}_1$ with two independent variables $X$ and $Y$ and network $\mathcal{G}_2$ with two independent variables and structure $X \rightarrow Y$. Then the likelihood scores are computed as follows:
\begin{eqnarray*}
...
The term $I_{\hat P}(X, Y)$ is called \term{mutual information} between $X$ and $Y$ and expresses the level of dependency of $Y$ on $X$. It can be proofed~\cite[p.~792]{pgm} that the overall likelihood score decomposes over a general graph $\mathcal{G}$ as follows:
\begin{equation}
    score_L(\mathcal{G} : \mathcal{D}) = N \!\!\!\!\!\! \sum_{X_i \in Nodes} \!\!\!\!\!\! I_{\hat P}(X_i;Parents(X_i)) - N \!\!\!\!\!\! \sum_{X_i \in Nodes} \!\!\!\!\!\! H_{\hat P}(X_i)
\end{equation}
where $H_{\hat P}(\cdot)$ is a constant relative to the structure of $\mathcal{G}$, so we really need to compare two network structures by the value of the first sum.

\medskip
The score difference between a superstructure with more edges and its substructure is always non-negative and, in fact, is equal to zero if and only if the two variables $X,Y$ in the dataset $\mathcal{D}$ appear to be perfectly independent which is due to statistical noise almost never true. Therefore the likelihood score itself almost always suggests the greedy heuristics to add an edge which would step by step lead to a fully connected network. This problem can be addressed by thresholding of the score increase or by restraining complexity of the network (maximal number of parents or maximal number of overall network parameters). Another approach is to account for network complexity in the scoring function itself, thus imposing a penalty on complicated structures. This is exactly what the BIC score does.



\subsection{Bayesian score}
The likelihood score discussed earlier never decides to pick a simpler structure over a more complex one. We will discuss a variant of Bayesian score called \term{BIC score} which is, in the end, just the likelihood score extended by a penalty term although theoretical foundations and mathematical derivations leading to the final formulas are entirely different.
...
\end{equation}
where $Dim[\mathcal{G}]$ is dimension of the network (number of its parameters) and $k$ is a constant relative to $\mathcal{G}$. We can see that the BIC score increases linearly in $N$ with dependent variables being connected (the first term) and decreases logarithmically in $N$ with the network complexity (the third term). Thus for a sparse dataset only the strong dependencies will be present in the final network, whereas for a large dataset the penalization increases rather slowly allowing the network to have a more complicated structure with weaker dependencies.

\todo{video, don't be afraid of gamma functions}
...
\begin{itemize}
	...
	\item direction don't matter, so we compute edge weights for every two nodes and find a maximum-weight spanning tree (Kruskal algorithm etc.) in $O(n^2)$
...
\begin{itemize}
	...
	\item traffic situation prediction
	\item protein networks (how a protein influences another protein)\,--\,"Learning General Graphs: Heuristic Search"
...
\begin{itemize}
	\item Study variants of stochastic sampling methods~\cite{neal93} if they seem to be needed.
	\item Another forms of structure learning (Bayesian learning\,--\,averages over many hypotheses).
\end{itemize}
