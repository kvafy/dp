...
Let's take a closer look at the hyperparameters. It is easy to see from formulas above that the relative difference between $\alpha$ and $N$ terms determines strength of the prior distribution $P(\Theta)$, ie. how many real samples accounted for in the $N$ counters does it take to significantly deviate from the prior distribution defined purely by hyperparameters $\alpha$.
...
Because the used dataset contains continuous features and because this thesis considers strictly discrete Bayesian networks we need to perform some kind of discretization. I have used binning with three equidepth bins labeled as low, medium and high. Such discretization seems reasonable wrt. to the relatively small dataset and especially because the values are, for a human, easily interpreted. Only three attributes \todo{xxx} are uneven (one bin is entirely empty) but these aren't significantly correlated with crime and will be omitted.
