...
\begin{itemize}
	...
	\item \todo{Solutions for effective inference and learning. As the previous chapters have mostly been of mathematical nature, at this point it might be useful to explain solutions of some of the problems in a more algorithmic way.}
\end{itemize}
...
\section{Exceptions}
In the whole \srccode{bnlib} library all exceptions are unmanaged with a single base exception \srccode{BNLibException}. Each method specifies a list of throwable exceptions via the \srccode{throws} keyword and its javadoc comment explains condition under which the listed exceptions may be thrown.
...
\todo{structure of .net files (see for yourself, \uv{net} section is ignored,) and .csv files (firs line is header containing variable names and complete list of values (even if no such value is presented in the particular dataset)}

\todo{maybe put query specification in this section}
...
\begin{figure}[ht]
    ...
    \begin{subfigure}[b]{0.45\linewidth}
        ...
        \caption{Using the BIC score. \todo{LungParench may be reversed and its descendants then too (more runs)}}
...
With a dataset of 2\,000 samples and the BIC score we get a connected graph in which only the edge $Sick - Age$ is missing and two other edges are reversed (see Figure~\ref{fig:structure-learning-child-2000-bic}). There are three chain structures involving the $Disease$ variable; legal variants of the chain structures are only those that don't form a V-structure in the $Disease$ variable, eg. if $Age$ is parent of $Disease$ then no other variable of the other two chain structures may be a parent of $Disease$. Additional four outcoming edges of the $Disease$ variable (painted in gray color) may be reversed if it doesn't introduce a V-structure in the $Disease$ variable which, again, depends on the concrete variant of the three chain structures. If you think about it, these reversible edges form with the $Disease$ variable a two-node chain. Furthermore, if the edge $Disease - LungParench$ is reversed then $LungParench - CO2 - CO2Report$ effectively become a chain structure with three variants. \todo{why are there so much more variants than with 500 samples, especially the gray edges? run with many runs (1000+)}

With the Bayesian score (best results achieved with $\alpha=2.5$, see Figure~\ref{fig:structure-learning-child-2000-bayes-2_5}) and 2\,000 samples the resulting graph is connected but its missing one $Sick - Age$, just as with the BIC score, and three edges are reversed. There is a single redundant connection $Disease - LVHreport$ which doesn't seem to be a big mistake because variables $LVH$ and $LVHreport$ are both descendants of $Disease$ and there are no other connections from these two nodes, so the error is local and doesn't affect any other node. We can make the observation that higher value of $\alpha$ means higher chance of the learnt network having an extra edge and also doesn't improve the number of reversed edges. So, $\alpha$ should be kept at a small value.
...
\medskip
Parameters of the experiments: 1\,000 runs, 600 iterations per run, 50 random restart steps, 0.15 relative size of tabu-list, maximum of 3 parents, $\alpha=1.5-3$.
...
%1\,000 samples: BIC (8, 5, 3), Bayesian $(\alpha=0.1, 2, 9, 5)$, $(\alpha=1, 2, 6, 5)$, $(\alpha=2, 2, 10, 7)$, $(\alpha=3, 2, 7, 9)$, $(\alpha=4, 2, 11, 13)$, $(\alpha=5, 3, 19, 20)$
%5\,000 samples: BIC (x, x, x), Bayesian $(\alpha=0.1, x, x, x)$, $(\alpha=1, x, x, x)$, $(\alpha=2, x, x, x)$, $(\alpha=3, x, x, x)$, $(\alpha=4, x, x, x)$, $(\alpha=5, x, x, x)$
%20\,000 samples: BIC (x, x, x), Bayesian $(\alpha=0.1, x, x, x)$, $(\alpha=1, x, x, x)$, $(\alpha=2, x, x, x)$, $(\alpha=3, x, x, x)$, $(\alpha=4, x, x, x)$, $(\alpha=5, x, x, x)$
\todo{xxx}
...
\begin{itemize}
    ...
    \item Best value of $\alpha$ for the Bayesian score usually, although not always, is that for which the Bayesian score of the best-scoring structure is the highest. This provides us with a powerful rule of thumb on how to pick $\alpha$ when learning a network structure whose original structure is unknown to us. \todo{verify, plot graphs for best score vs $\alpha$}
\end{itemize}
