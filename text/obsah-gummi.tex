...
The main problem we must face is that no alteration may introduce a directed cycle into the network because Bayesian networks are purely DAGs.
Let $r$ be binary relation over the set of all nodes such that $\forall X,Y: XrY \Leftrightarrow X \in Parents(Y)$. As usual, $r^+$ will denote transitive closure of the relation $r$.
Lets now inspect three the possible alteration cases (we always consider arbitrary variables $X,Y$ such that $X \neq Y$).
...
To grasp the problem of edge reversion we will need to make use of a topological sort of the given network. Let $\prec$ be total ordering induced by a topological sort of our network. Then for all nodes we can write $A_1 \prec \dots \prec A_a \prec X \prec M_1 \prec \dots \prec M_m \prec Y \prec B_1 \prec \dots \prec B_b$ which can be written using sets of variables as $\vars{A} \prec X \prec \vars{M} \prec Y \prec \vars{B}$, any of the sets $\vars{A}$, $\vars{M}$, $\vars{B}$ may be empty.

Lets think about what would happen if the edge $(X,Y)$ was reversed? First, we would have to change the relative order of $X$ and $Y$ in the $\prec$ ordering to obtain a new ordering $\prec'$ for the new network. Suppose that we just swap positions of $X$ and $Y$ in the original ordering. Now the question is what conditions have to be met for $\vars{A} \prec' Y \prec' \vars{M} \prec' X \prec' \vars{B}$ to be a valid topological ordering of the new network? In other words, under which conditions is the new network acyclic (a~graph has a topological ordering iff it is a DAG)?
...
Classes with common ancestor \srccode{SampleProducer} described in the previous section are able to generate a single sample on demand using a specific sampling technique. To provide an approximation of a general distribution $P(\vars{X} \mid \vars{Y}, \vars{e})$ we use an instance of \srccode{QuerySampler} which takes a \srccode{SampleProducer}, generates a requested number of samples and, using statistics, computes an approximation of the desired probability distribution in form of a factor.

The internal logic of \srccode{QuerySampler} is very simple and straightforward\,--\,get an instance of \srccode{SampleProducer} in the constructor, in \srccode{sample(...)} method create an instance of \srccode{SamplingContext}, initialize the context and use the sample producer along with the created context to generate a large number of samples. Each sample is accounted for in corresponding counter and in the end we can obtain the statistics as a \srccode{Factor} instance via methods \srccode{getSamplesCounter()} and \srccode{getSamplesCounterNormalized()}; return value of the latter represents the approximated probability distribution $P(\vars{X} \mid \vars{Y}, \vars{e})$.
...
\begin{itemize}
	\item I myself devised some proofs: Joint probability distribution induces by a BN indeed is a probability distribution (sums to one). Derivation of the formula for weighted sampling.
\end{itemize}
%=========================================================================
