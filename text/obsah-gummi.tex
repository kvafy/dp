...
This chapter will present theoretical foundations for understanding Bayes networks and for doing computations over them. Studying Bayes networks (further abbreviated as BNs) is challenging both because of necessary mathematical rigor as well as for the need of efficient non-trivial algorithms to perform inference given some observations.

First will be presented notation used in this thesis and necessary overview of probability theory. Then we will proceed to theory of Bayes networks, at first in terms of pure mathematics followed by techniques of practical and effective \uncertain{reasoning about information represented by them according to some observation.}




\section{Preliminaries}
At first we need to establish notation and explain basic facts regarding mathematical probability because, as will be shown later, Bayes networks are graphical models for joint probability distributions.
...
\begin{math_def}
    Let $G$ be a Bayes network, $X,Y$ two of its nodes and $Z$ facts (observations). We say that $X,Y$ given $Z$ are d-separated in $G$ (denoted $\text{d-sep}_G(X,Y|Z)$) iff there is no active trail between $X$ and $Y$ given $Z$.
\end{math_def}
...
Sum-product inference method has, from its nature, exponential running time in the worst case. It depends on number of all nodes (variables) and on the number of possible assignments to a variable. There might be other factors involved but, as sum-product is just a method rather than algorithm, they won't be discussed.
